...
def main():
    ...
    if download == "w":
        ...
        to_year = f"{int(to_year.strftime('%Y'))}"

        for date_time_variable in range(int(from_year), int(to_year)):
            url = f"https://www.boxofficemojo.com/year/world/{date_time_variable}/"
            timeperiod_y = str(date_time_variable)[:4]
            print(timeperiod_y)
            print(f'This is the name of the url:  {url}')
            movie_dollar: pd.DataFrame = pd.read_html(url)[0]
            movie_dollar['Year #'] = date_time_variable
            movie_dollars.append(movie_dollar)
